...
internal class AndroidAuthSessionManager(
    ...
    fun beginTelegramBot(
        command: TelegramAuthorizationCommand,
    ): ApiResult<TelegramBotAuthorization> = synchronized(lock) {
        val token = validTokenLocked()
            ?: return@synchronized ApiResult.Failure(ApiError.AuthenticationRequired())
