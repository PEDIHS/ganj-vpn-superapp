 ...
 * The envelope is authenticated with Android Keystore AES-GCM and is never exposed through an
 * Intent, URI, QR, clipboard, file import or public serializer.
 */
class SecureProfileRecoveryStore(context: Context) {
    ...
    fun save(profile: ProvisionedProfile): Result<Unit> {
        ...
        return try {
            ...
            check(preferences.edit().putString(PROFILE_PREFERENCE, encoded).commit()) {
