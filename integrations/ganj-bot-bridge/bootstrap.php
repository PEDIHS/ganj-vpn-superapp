 ...
 * No bot token, database credential, VPN configuration or Control API credential is embedded here.
 */
...
function ganjAppBridgeCanonicalJson(array $payload): string
{
...
function ganjAppBridgeRequest(string $decision, string $approvalToken): bool
{
    ...
    if (!in_array($decision, ['approve', 'deny'], true)) throw new RuntimeException('Approval decision is invalid.');
    if (!preg_match('/^[A-Za-z0-9_-]{43}$/D', $approvalToken)) {
        throw new RuntimeException('Approval token is invalid.');
    }
    ...
    $callbackId = (string)($GLOBALS['callback_query_id'] ?? '');
    $eventId = 'tgcb:' . substr(hash('sha256', $callbackId !== '' ? $callbackId : ($fromId . ':' . $approvalToken . ':' . $decision)), 0, 40);
    $payload = [
        'approval_token' => $approvalToken,
        'decision' => $decision,
        'event_id' => $eventId,
        'telegram_display_name' => $decision === 'approve' ? $displayName : null,
        'telegram_subject' => $decision === 'approve' ? $fromId : null,
        'telegram_username' => $decision === 'approve' ? $username : null,
    ];
    $encoded = ganjAppBridgeCanonicalJson($payload);
    $timestamp = (string)time();
    $secret = ganjAppBridgeSecret((string)($config['approval_hmac_secret_file'] ?? ''));
    ...
    curl_setopt_array($handle, [
        ...
        CURLOPT_HTTPHEADER => [
            ...
            'X-Ganj-Bot-Timestamp: ' . $timestamp,
            'X-Ganj-Bot-Signature: ' . $signature,
        ],
        ...
        CURLOPT_USERAGENT => 'ganj-bot-app-bridge/2.0',
    ]);
    try {
        ...
        return is_array($body) && empty($body['error']) && !empty($body['data']['accepted']);
    } catch (Throwable $error) {
        return false;
    } finally {
        curl_close($handle);
        if (isset($secret)) $secret = str_repeat("\0", strlen($secret));
        if (isset($signature)) $signature = str_repeat("\0", strlen($signature));
        if (isset($approvalToken)) $approvalToken = str_repeat("\0", strlen($approvalToken));
    }
}
...
function ganjAppBridgeHandle(): bool
{
    ...
    if (preg_match('/^\/start\s+ga_([A-Za-z0-9_-]{43})$/D', $text, $match)) {
        $token = $match[1];
        $keyboard = json_encode([
            'inline_keyboard' => [
                [['text' => '✅ تأیید ورود به اپ گنج', 'callback_data' => 'ga_a_' . $token]],
                [['text' => '❌ رد درخواست', 'callback_data' => 'ga_d_' . $token]],
            ],
        ], JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
        sendmessage(
            $GLOBALS['from_id'],
            "📱 <b>ورود به اپلیکیشن Ganj VPN</b>\n\nاین درخواست فقط حساب تلگرام فعلی را به نشست امن همین دستگاه در اپ متصل می‌کند. گنج هیچ کد ورود، رمز تلگرام یا اطلاعات کانفیگ از شما نمی‌خواهد.\n\n<b>اگر خودتان این درخواست را شروع نکرده‌اید، آن را رد کنید.</b>",
            $keyboard,
            'HTML'
        );
    ...
    if (!preg_match('/^ga_([ad])_([A-Za-z0-9_-]{43})$/D', $callback, $match)) return false;
    $decision = $match[1] === 'a' ? 'approve' : 'deny';
    try {
        $ok = ganjAppBridgeRequest($decision, $match[2]);
    } catch (Throwable $error) {
        $ok = false;
    }
    if (!empty($GLOBALS['callback_query_id'])) {
        telegram('answerCallbackQuery', [
            ...
            'text' => $ok
                ? ($decision === 'approve' ? 'ورود تأیید شد.' : 'درخواست رد شد.')
                : 'درخواست معتبر نیست، منقضی شده یا سرویس موقتاً در دسترس نیست.',
            'show_alert' => !$ok,
        ]);
    }
    $message = $ok
        ? ($decision === 'approve'
            ? "✅ <b>ورود به اپ تأیید شد</b>\n\nبه اپ برگردید؛ وضعیت تأیید بررسی می‌شود و سرویس‌های حساب به‌صورت خودکار تازه‌سازی می‌شوند."
            : "❌ درخواست ورود به اپ رد شد.")
        : "❌ <b>ثبت نتیجه ورود انجام نشد</b>\n\nممکن است درخواست منقضی شده باشد. از داخل اپ دوباره تلاش کنید.";
    Editmessagetext(
        $GLOBALS['from_id'],
        $GLOBALS['message_id'],
        $message,
