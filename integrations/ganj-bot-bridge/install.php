...
$approvalHmacSecretFile = installerPrivateSecretFile(installerEnv('GANJ_BOT_APPROVAL_HMAC_SECRET_FILE'), $root);
...
$config = [
    ...
    'approval_hmac_secret_file' => $approvalHmacSecretFile,
...
if (!str_contains($original, $managed)) {
    ...
    try { $backupSuffix = bin2hex(random_bytes(6)); }
    catch (Throwable $error) { installerFail('Cannot create a unique backup name.'); }
...
fwrite(STDOUT, "Telegram Bot Approval: HMAC signed, one-time and device-bound.\n");
