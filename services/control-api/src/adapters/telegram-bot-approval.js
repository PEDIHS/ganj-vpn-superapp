...
export class TelegramBotApprovalAdapter {
  constructor({
    ...
    if (!store || ['create', 'approve', 'cancel', 'findStatus', 'loadForExchange', 'markConsumed']
    ...
    if (!TELEGRAM_USERNAME.test(botUsername ?? '')) throw new Error('TELEGRAM_BOT_USERNAME is invalid.');
  ...
  async start({ principal, codeChallenge, redirectUri }) {
    if (!principal || typeof principal.userId !== 'string' || typeof principal.deviceId !== 'string') {
      throw new ApiError(401, 'telegram_bot_login_requires_session', 'A valid device session is required.');
    }
    ...
    const expiresAt = new Date(this.clock().getTime() + this.stateSeconds * 1000);
  ...
  async decide({ authorization, requestToken, action, telegramUserId, username, displayName }) {
    ...
    if (action === 'cancel') {
      const cancelled = await this.store.cancel({ requestDigest: digest(requestToken), now: this.clock() });
      if (!cancelled) throw new ApiError(409, 'telegram_bot_request_unavailable', 'Telegram Bot approval request is unavailable.');
    ...
    if (!approved) throw new ApiError(409, 'telegram_bot_request_unavailable', 'Telegram Bot approval request is unavailable.');
  ...
  async status({ principal, state }) {
    ...
    if (!row) throw new ApiError(404, 'telegram_bot_state_not_found', 'Telegram Bot approval state was not found for this device.');
    ...
    if (row.status !== 'approved') return { status: 'pending', expires_at: row.expiresAt.toISOString() };
    return {
      status: 'approved',
  ...
  async exchangeAuthorizationCode({ code, state, codeVerifier }) {
    if (!BASE64URL_32.test(code ?? '') || !BASE64URL_32.test(state ?? '')
    ...
    const row = await this.store.loadForExchange({ stateDigest, now: this.clock() });
    if (!row || row.status !== 'approved') {
      throw new ApiError(400, 'invalid_telegram_bot_exchange', 'Telegram Bot approval is invalid, expired, or unavailable.');
    }
    ...
    const consumed = await this.store.markConsumed({ stateDigest, now: this.clock() });
    if (!consumed) throw new ApiError(409, 'telegram_bot_exchange_replayed', 'Telegram Bot exchange was already consumed.');
...
export class PostgresTelegramBotApprovalStore {
  constructor(pool) {
    if (typeof pool?.connect !== 'function' || typeof pool?.query !== 'function') throw new Error('PostgreSQL pool is required.');
    this.pool = pool;
  }
  ...
  async findStatus({ stateDigest, userId, deviceId, now }) {
    const result = await this.pool.query(
      `SELECT state_digest, code_challenge, device_id, telegram_subject,
  ...
  async loadForExchange({ stateDigest, now }) {
    const result = await this.pool.query(
      `SELECT state_digest, code_challenge, device_id, telegram_subject,
              telegram_username, telegram_display_name, expires_at,
              approved_at, cancelled_at, consumed_at
         FROM control_telegram_bot_approvals WHERE state_digest = $1`,
      [stateDigest],
    );
    if (result.rowCount !== 1) return null;
    return mapRow(result.rows[0], now);
  ...
  async markConsumed({ stateDigest, now }) {
    const result = await this.pool.query(
      `UPDATE control_telegram_bot_approvals
          SET consumed_at = $2
        WHERE state_digest = $1 AND approved_at IS NOT NULL
          AND cancelled_at IS NULL AND consumed_at IS NULL AND expires_at > $2
...
export async function createTelegramBotApprovalAdapter({ environment = process.env } = {}) {
  ...
  return new TelegramBotApprovalAdapter({
    ...
    redirectUris: environment.TELEGRAM_BOT_REDIRECT_URIS.split(',').map((item) => item.trim()).filter(Boolean),
